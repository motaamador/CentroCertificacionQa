...
import FnTester from "@/components/tabs/FnTester";
import UserManager from "@/components/tabs/UserManager";
...
export default function AppClient({ user }) {
  ...
  const render = () => {
    switch (activeTab) {
      ...
      case "fn-tester":    return <FnTester />;
  ...
  return (
    <Shell activeTab={activeTab} onTabChange={setActiveTab} user={user}>
      {render()}
    </Shell>
  );
}
